...
describe("Parse Numbers", () => {
    ...
    if (typeof BigInt === "function") {
        it("should parse a bigint number literal as expected", () => {
            assert.strictEqual(parse(getData("literal-number-bigint")), BigInt(12345));
        });

        it("should parse a bigint number literal with plus-sign mark as expected", () => {
            assert.strictEqual(parse(getData("literal-number-bigint-plus")), BigInt(12345));
        });

        it("should parse a bigint number literal with minus-sign mark as expected", () => {
            assert.strictEqual(parse(getData("literal-number-bigint-minus")), BigInt(-12345));
        });
    }
});